...
public class CachedGroupMemberRepository implements GroupMemberRepository {
    ...
    @Override
    @Cached(name = CacheNames.GROUP_MEMBERS, key = "#groupId.value()", cacheType = CacheType.REMOTE,
    ...
    @Override
    @Cached(name = CacheNames.GROUP_MEMBER_CONTAIN, key = "#groupId.value() + ':' + #userId.value()",
    ...
    @Override
    @CacheInvalidate(name = CacheNames.GROUP_MEMBERS, key = "#member.getGroupId().value()")
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER, key = "#member.getGroupId().value() + ':' + #member.getUserId().value()")
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER_CONTAIN, key = "#member.getGroupId().value() + ':' + #member.getUserId().value()")
    public void save(GroupMember member) {
        delegate.save(member);
    }

    @Override
    @CacheInvalidate(name = CacheNames.GROUP_MEMBERS, key = "#members.![getGroupId().value()]", multi = true)
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER, key = "#members.![getGroupId().value() + ':' + getUserId().value()]", multi = true)
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER_CONTAIN, key = "#members.![getGroupId().value() + ':' + getUserId().value()]", multi = true)
    public void save(List<GroupMember> members) {
        delegate.save(members);
    }

    @Override
    @CacheInvalidate(name = CacheNames.GROUP_MEMBERS, key = "#groupMember.getGroupId().value()")
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER, key = "#groupMember.getGroupId().value() + ':' + #groupMember.getUserId().value()")
    @CacheInvalidate(name = CacheNames.GROUP_MEMBER_CONTAIN, key = "#groupMember.getGroupId().value() + ':' + #groupMember.getUserId().value()")
