...
const consoles: Record<ConsoleKey, Omit<ConsoleLink, "key">> = {
  ...
  audit: {
    name: "审计中心",
    location:
      import.meta.env.VITE_AUDIT_CONSOLE_URL || "http://localhost:18091",
  },
  monitor: {
    name: "运行监控",
    location:
      import.meta.env.VITE_MONITOR_CONSOLE_URL || "http://localhost:18092",
  },
  admin: {
    name: "用户管理",
    location:
      import.meta.env.VITE_ADMIN_CONSOLE_URL || "http://localhost:18093",
  },
};

export function consoleLinks(current: ConsoleKey): ConsoleLink[] {
  return (
    Object.entries(consoles) as Array<[ConsoleKey, Omit<ConsoleLink, "key">]>
  )
