...
UPDATE `db_iam_oauth_client`
SET `redirect_uris` = JSON_ARRAY('http://localhost:18090/iam/callback'),
    `post_logout_redirect_uris` = JSON_ARRAY('http://localhost:18090/'),
    `update_time` = CURRENT_TIMESTAMP(3)
WHERE `oauth_client_id` = 'im-monitor-client'
  AND `redirect_uris` = JSON_ARRAY('http://localhost:18092/iam/callback')
  AND `post_logout_redirect_uris` = JSON_ARRAY('http://localhost:18092/')
  AND `is_deleted` = 0;
