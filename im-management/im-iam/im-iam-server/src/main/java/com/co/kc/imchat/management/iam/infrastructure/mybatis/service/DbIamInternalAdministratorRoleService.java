package com.co.kc.imchat.management.iam.infrastructure.mybatis.service;

import com.co.kc.imchat.management.iam.infrastructure.mybatis.entity.DbIamInternalAdministratorRole;
import com.co.kc.imchat.management.iam.infrastructure.mybatis.mapper.DbIamInternalAdministratorRoleMapper;
import com.co.kc.imchat.plugin.datasource.dao.BaseMybatisService;
import org.springframework.stereotype.Service;

/** IAM 管理员内部角色关联表 MyBatis 服务。 */
@Service
public class DbIamInternalAdministratorRoleService
        extends BaseMybatisService<DbIamInternalAdministratorRoleMapper, DbIamInternalAdministratorRole> {
}
