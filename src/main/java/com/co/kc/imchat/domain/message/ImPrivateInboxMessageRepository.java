...
public interface ImPrivateInboxMessageRepository {
    ...
    ImPrivateInboxMessage find(ImChatId chatId, ImMessageId messageId);

    ImPrivateInboxMessage find(ImChatId chatId, ImMessageToken messageToken);

    List<ImPrivateInboxMessage> queryHistory(ImChatId imChatId, ImMessageId imLastMessageId, int count, UserId viewer);

    ImPrivateInboxMessage queryDetail(ImChatId chatId, ImMessageToken messageToken, UserId viewer);
