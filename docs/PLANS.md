...
计划还必须包含有序实施任务，以及满足验收标准所需的准确测试与验证策略。状态表只提供任务级索引，不能替代详细任务：每个任务 ID 必须有同 ID 的三级标题、明确文件范围、可执行的小步骤和至少一个 Markdown checkbox。完成步骤立即由 `[ ]` 改为 `[x]`；任务只有在全部步骤勾选且验证证据有效时才能标记为 `passing`。

详细任务按测试先行顺序描述精确动作。适用时至少写清：新增或修改的准确文件、RED 测试及预期失败、最小实现、GREEN 验证命令、文档与回滚处理。配置或纯文档工作无法合理使用 TDD 时，也必须提供可重复的契约检查或人工 Review 条件，不能只写“完成开发”“运行测试”等不可恢复描述。
...
2. 实施前写明 Sprint Contract、任务看板、逐项 checkbox、恢复状态和准确验证命令，并把计划登记到根 `PROGRESS.md`。
3. 随工作进展立即勾选完成步骤，并更新任务状态、恢复状态和 PROGRESS 摘要；偏离预期设计时记录变化。
